...
const shaList = revs => {
  Object.keys(revs.refs).forEach(ref => {
    doc = revs.refs[ref]
    if (revs.shas[doc.sha]) { revs.shas[doc.sha].push(ref) } else { revs.shas[doc.sha] = [ref] }
  })
  return revs
}
...
const distTags = revs => {
  ...
  versions.forEach(v => {
    ...
    if (revs.refs.latest && ver.sha === revs.refs.latest.sha) { revs['dist-tags'].latest = v } else if (ver.sha === HEAD.sha) {
...
const linesToRevsReducer = (revs, line) => {
  ...
  if (doc.type === 'tag') {
    ...
    if (match && semver.valid(match[1], true)) { revs.versions[semver.clean(match[1], true)] = doc }
  }
